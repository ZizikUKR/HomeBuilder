...
{
    public class SpendRepository : BaseGuidRepository<Spend>, ISpendRepository
    {
        public SpendRepository(IConfiguration configuration) : base(configuration, "Spends")
        {
        }

        public async Task<IEnumerable<Spend>> GetAllPagedWithFilters(int skippedItems, int pageSize, string category, int year, int month)
        {
            var sql = new StringBuilder($@"SELECT * FROM {_tableName} AS FP
                            INNER JOIN SpendCategories AS FC ON FP.CategoryId =FC.Id
                            WHERE Year=@Year");
            ...
            var res = await Connection.QueryAsync<Spend, SpendCategory, Spend>(sql.ToString(), map: (p, c) =>
            {
                p.Category = c;
                return p;
